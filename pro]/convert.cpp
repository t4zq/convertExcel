...
// 科学（指数）表記が妥当かの自動判定（|x| >= 1e4 または |x| < 1e-3）
...
// mantissa × 10^{exp} を LaTeX 数式で生成（mantissa は有効数字ベースで整形）
static std::string to_scientific_latex(double value, int sig_figs) {
  if (sig_figs <= 0) sig_figs = 1;
  if (value == 0.0) return "$0$";
  double a = std::abs(value);
  int exponent = (int)std::floor(std::log10(a));
  double mantissa = value / std::pow(10.0, exponent);
  // mantissa を有効数字で整形（1桁目 + 小数 sig_figs-1 桁）
  int decimal_places = std::max(0, sig_figs - 1);
  std::ostringstream oss;
  ...
  if (decimal_places > 0) {
    // trim trailing zeros
    while (!m_str.empty() && m_str.back() == '0') m_str.pop_back();
    if (!m_str.empty() && m_str.back() == '.') m_str.pop_back();
  }
  // 数式モードで生成（escape は適用しない）
  std::string out = "$" + m_str + " \\times 10^{" + std::to_string(exponent) + "}$";
  return out;
}

// 表のみに指数表記を自動適用（数値セルのみ）。非対象の数値は有効数字で通常表記、文字列は escape。
std::string to_latex_scientific_auto(const Table &t, int sig_figs) {
  if (t.empty()) return "";
    ...
    for (size_t i = 0; i < row.size(); ++i) {
      ...
      if (is_number(cell)) {
        ...
        if (should_use_scientific(v)) {
          out += to_scientific_latex(v, sig_figs);
        } else {
          // 通常表記（有効数字で整形）し、文字列として escape
          std::string rounded = round_significant_figures(cell, sig_figs);
          out += escape(rounded);
        }
      } else {
        out += escape(cell);
...
EMSCRIPTEN_KEEPALIVE char* gen_latex_sig_figs(const char* in, int sig_figs) {
  if (!in) return dup("");
  // sig_figs が負の場合は「表のみ指数表記（自動）」モード
  if (sig_figs < 0) {
    int sf = -sig_figs;
    return dup(to_latex_scientific_auto(parse(in), sf));
  }
  return dup(to_latex_sig_figs(parse(in), sig_figs));
}
